...
3. `~/MEGA/work-with-claude-code/classes/ENGL-1181/CLAUDE.md` — this course: term, sections, readings, assignments, calendars.
...
`~/MEGA/work-with-claude-code/classes/ENGL-1181/changelog/`

**One HTML file per date, named `YYYY-MM-DD.html`**, plus an `index.html` listing them all with a short summary each. Add both **in the same session as the change**, not later — the reasoning is what the log is for, and it evaporates fast.
...
`calendar/calendar.js` and `calendar/calendar.css` are **byte-identical** in `1181`, `1170` and `1190`. Not similar; identical. Check before and after any change:

```
md5sum ~/Websites_work/{1181,1170,1190}/calendar/calendar.{js,css}
```
